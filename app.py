...
from utils1 import (
    init_audit,
    log_query,
    fetch_audit_for_role,
    audit_to_csv_bytes_for_role,
    SchemaFormatter,
...
# ======================================================
# DataFrame column deduplication helper
# ======================================================
...
# ======================================================
# Whitelisted DBs (persisted) — Guests restricted to this
# ======================================================
ALLOWED_DBS_FILE = ".data/allowed_dbs.json"
os.makedirs(".data", exist_ok=True)
DEFAULT_ALLOWED_DBS = ["chinook", "northwind"]

def load_allowed_dbs() -> list:
    try:
        if os.path.exists(ALLOWED_DBS_FILE):
            with open(ALLOWED_DBS_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
                if isinstance(data, list):
                    return data
        env_list = [s.strip() for s in os.getenv("ALLOWED_DBS", "").split(",") if s.strip()]
        return env_list or DEFAULT_ALLOWED_DBS
    except Exception:
        return DEFAULT_ALLOWED_DBS

def save_allowed_dbs(db_list: list):
    try:
        with open(ALLOWED_DBS_FILE, "w", encoding="utf-8") as f:
            json.dump(sorted(set(db_list)), f, indent=2)
    except Exception:
        pass

# ======================================================
# Per-DB visibility rules: which tables are accessible
# ======================================================
VIS_RULES_FILE = ".data/visibility_rules.json"

def _load_visibility_rules() -> dict:
    if os.path.exists(VIS_RULES_FILE):
        try:
            with open(VIS_RULES_FILE, "r", encoding="utf-8") as f:
                return json.load(f)
        except Exception:
            return {}
    return {}

def _save_visibility_rules(rules: dict):
    try:
        with open(VIS_RULES_FILE, "w", encoding="utf-8") as f:
            json.dump(rules, f, indent=2)
    except Exception:
        pass

def _suggest_sensitive_tables(all_tables: list) -> list:
    patterns = [
        r"customer", r"account", r"transaction", r"card", r"cvv", r"aadhaar", r"pan",
        r"ssn", r"salary", r"payroll", r"user", r"identity"
    ]
    sens = []
    for t in all_tables:
        lt = t.lower()
...
def _filter_schema(schema: dict, allowed_tables: list) -> dict:
    if not allowed_tables:
        return schema
    allowed_set = {t.lower() for t in allowed_tables}
    return {t: meta for t, meta in schema.items() if t.lower() in allowed_set}

# ======================================================
# Zero-LLM query suggestions based on schema
# ======================================================

def _first(cols, pred, default=None):
    for c in cols:
        if pred(c):
            return c
    return default

def generate_schema_suggestions(schema: dict, normal=5, advanced=5) -> dict:
    suggestions = {"normal": [], "advanced": []}
    if not schema:
        return suggestions
    ...
    def cols_of(t):
        return [c.get("name") for c in schema[t].get("columns", [])]

    def dtype_of(t, c):
        for col in schema[t].get("columns", []):
            if col.get("name") == c:
                return str(col.get("type") or "").lower()
        return ""

    # Normal suggestions
    for t in tables:
        suggestions["normal"].append(f"Show the first 10 rows from {t}")
        suggestions["normal"].append(f"Count rows in {t}")
        cands = cols_of(t)
        txt = _first(cands, lambda c: c and ("name" in c.lower() or "type" in c.lower() or "category" in c.lower()))
        num = _first(cands, lambda c: c and ("int" in dtype_of(t, c) or "real" in dtype_of(t, c) or "num" in dtype_of(t, c)))
        if txt and num:
            suggestions["normal"].append(f"Show total of {num} by {txt} in {t}, top 10")
        dt = _first(cands, lambda c: c and ("date" in c.lower() or "time" in c.lower()))
        if dt and num:
            suggestions["normal"].append(f"Show monthly trend of {num} over {dt} in {t}")
        if len(suggestions["normal"]) >= normal:
            break

    # Advanced suggestions (joins)
    for t in tables:
        fks = schema[t].get("foreign_keys", [])
        if not fks:
            continue
        fk = fks[0]
        ...
        if ref_t not in schema:
            continue
        t_cols = cols_of(t)
        r_cols = cols_of(ref_t)
        t_name = _first(t_cols, lambda c: c and ("name" in c.lower())) or (t_cols[0] if t_cols else "id")
        r_name = _first(r_cols, lambda c: c and ("name" in c.lower())) or (r_cols[0] if r_cols else "id")
        suggestions["advanced"].append(
            f"Join {t} with {ref_t} and list {t}.{t_name} along with {ref_t}.{r_name}, top 10"
        )
        if len(suggestions["advanced"]) >= advanced:
            break

    # Fill remaining advanced
    i = 0
    while len(suggestions["advanced"]) < advanced and i < len(tables):
        t = tables[i]
        cands = cols_of(t)
        num = _first(cands, lambda c: c and ("int" in dtype_of(t, c) or "real" in dtype_of(t, c) or "num" in dtype_of(t, c)))
        if num:
            suggestions["advanced"].append(f"Find the top 10 records in {t} by {num}")
        else:
            suggestions["advanced"].append(f"Show distinct values of columns in {t}")
        i += 1

    suggestions["normal"] = list(dict.fromkeys(suggestions["normal"]))[:normal]
    suggestions["advanced"] = list(dict.fromkeys(suggestions["advanced"]))[:advanced]
    return suggestions


def render_suggestions_box(suggestions: dict):
    normal = suggestions.get("normal", [])
    advanced = suggestions.get("advanced", [])
    st.markdown(
        """
        <style>
        .sugg-card {border:1px solid #e0e0e0;border-radius:12px;padding:12px 14px;max-height:260px;overflow-y:auto;background:#fafafa}
        .sugg-title {font-weight:600;margin-top:6px}
        .sugg-item {margin:6px 0;padding:6px 8px;background:#fff;border-radius:8px;border:1px solid #eee}
        </style>
        """,
        unsafe_allow_html=True,
    )
    st.markdown('<div class="sugg-card">', unsafe_allow_html=True)
    st.markdown('<div class="sugg-title">Normal queries</div>', unsafe_allow_html=True)
    for q in normal:
        st.markdown(f'<div class="sugg-item">• {q}</div>', unsafe_allow_html=True)
    st.markdown('<div class="sugg-title" style="margin-top:10px;">Advanced queries</div>', unsafe_allow_html=True)
    for q in advanced:
        st.markdown(f'<div class="sugg-item">• {q}</div>', unsafe_allow_html=True)
...
# --- Restored UI and lifecycle logic from app2.py ---

    with st.sidebar:
        st.header("🗂️ Database")
        allowed_dbs = load_allowed_dbs()
        if is_guest():
            if not allowed_dbs:
                st.warning("No databases are available. Please contact a Manager/Admin to whitelist one.")
                db_choice = None
                connect_enabled = False
            else:
                db_choice = st.selectbox("Select a database", allowed_dbs, index=0)
                connect_enabled = True
            st.caption("Guests can use only whitelisted databases. Contact Manager/Admin to request access.")
        else:
            mode = st.radio("DB source", ["Whitelist", "Custom path"], horizontal=True)
            if mode == "Whitelist":
                db_choice = st.selectbox("Select a database", allowed_dbs, index=0 if allowed_dbs else None)
            else:
                db_choice = st.text_input("DB name/path", "chinook")
            connect_enabled = True

        if st.button("Connect", disabled=not connect_enabled or not db_choice):
            try:
                engine = create_engine_from_uri(db_choice)
                schema = get_schema(engine)
                all_tables = sorted(schema.keys(), key=str.lower)
                vis_rules = _load_visibility_rules()
                db_key = db_choice.lower()
                approved_tables = vis_rules.get(db_key)
                if not approved_tables:
                    suggested_sensitive = _suggest_sensitive_tables(all_tables)
                    approved_tables = [t for t in all_tables if t not in suggested_sensitive]
                effective_schema = _filter_schema(schema, approved_tables) if approved_tables else schema
                st.session_state["engine"] = engine
                st.session_state["schema"] = schema
                st.session_state["approved_tables"] = approved_tables
                st.session_state["db_choice"] = db_choice
                st.session_state["effective_schema"] = effective_schema
                st.session_state["schema_text"] = SchemaFormatter(effective_schema).formatted()
        ...
        if allow_audit_read():
            ...
            csv_bytes = audit_to_csv_bytes_for_role(current_role(), limit=10000)
...
with left_col:
    ...
    if "effective_schema" in st.session_state:
        show_suggestions = st.checkbox("Show AI suggestions (uses LLM quota)", value=False)
        if show_suggestions:
            try:
                nl2sql_sugg = NL2SQLAgent(st.session_state["effective_schema"])
                suggestions = []
                try:
            ...
            for q in suggestions:
                st.markdown(f"- {q}")
        # Zero-LLM suggestions
        suggestions = generate_schema_suggestions(st.session_state["effective_schema"])
        render_suggestions_box(suggestions)
    else:
        st.info("Connect a database to see suggestions.")
...
with right_col:
            ...
            try:
                ...
                if df is not None and isinstance(df, pd.DataFrame) and not df.empty:
                    if st.session_state["active_view"] == "Results":
                        result_display.dataframe(deduplicate_columns(df), width='stretch')
                        try:
                            log_query(
                                user=st.session_state["user"], role=current_role(),
    ...
    if allow_audit_read():
        audit_df = fetch_audit_for_role(current_role(), limit=100)
        st.dataframe(deduplicate_columns(audit_df), width='stretch')
    else:
...
if st.session_state["nav"] != "Workspace":
    ...
    # Users table
    st.subheader("Users")
    st.dataframe(deduplicate_columns(df), width='stretch', hide_index=True)
