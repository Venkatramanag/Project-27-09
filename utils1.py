...
# Retention & hashing settings (env-configurable)
AUDIT_RETENTION_DAYS = int(os.getenv("AUDIT_RETENTION_DAYS", "90"))
SQL_HASH_SECRET = os.getenv("SQL_HASH_SECRET", "change-this-secret-in-prod").encode("utf-8")
...
# ======================================================
# 🔐 Role-aware SQL masking
# ======================================================
def _sql_fingerprint(sql: str) -> str:
    h = hashlib.sha256(SQL_HASH_SECRET + (sql or "").encode("utf-8")).hexdigest()
    return h[:10]

def _mask_sql_for_role(sql: Optional[str], role: str) -> str:
    """Managers see full SQL. Others see masked with a stable fingerprint."""
    if sql is None:
        return ""
    if (role or "").strip().lower() == "manager":
        return sql
    return f"[hidden] (sha256:{_sql_fingerprint(sql)})"
...
def log_query(user: str, role: str, user_q: str, sql: str, status: str, rows: int, duration: float):
    """Insert a query log record into the audit DB (with PII redaction)."""
    conn = sqlite3.connect(AUDIT_DB, check_same_thread=False)
    cur = conn.cursor()
    ts = datetime.utcnow().isoformat()
    # Redact potential PII in user text and SQL (emails/phones only)
    user_q_red = _redact(user_q)
    sql_red = _redact(sql)
    cur.execute(
        "INSERT INTO query_audit (ts, user, role, user_query, sql, status, rows, duration) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
        (ts, user, role, user_q_red, sql_red, status, rows, duration)
    )
    conn.commit()
    conn.close()

def purge_audit_older_than(days: int = AUDIT_RETENTION_DAYS):
    """Delete audit rows older than N days."""
    try:
        cutoff = pd.Timestamp.utcnow() - pd.Timedelta(days=days)
        conn = sqlite3.connect(AUDIT_DB, check_same_thread=False)
        cur = conn.cursor()
        cur.execute("DELETE FROM query_audit WHERE ts < ?", (cutoff.isoformat(),))
        conn.commit()
        conn.close()
    except Exception:
        pass
...
# ======================================================
# 📜 Audit Fetching (role-aware)
# ======================================================
def fetch_audit_for_role(role: str, limit: int = 100) -> pd.DataFrame:
    """
    Fetch recent audit logs, masking SQL for non-managers.
    Returns a DataFrame with privacy masking applied.
    """
    conn = sqlite3.connect(AUDIT_DB, check_same_thread=False)
    q = f"""
        SELECT ts, user, role, user_query, sql, status, rows, duration
        FROM query_audit
        ORDER BY id DESC
        LIMIT ?
    """
    ...
    conn.close()
    # Mask SQL for non-managers
    if role.strip().lower() != "manager":
        df["sql"] = df["sql"].apply(lambda s: _mask_sql_for_role(s, role))
    return df

def audit_to_csv_bytes_for_role(role: str, limit: int = 10000) -> bytes:
    """
    Return audit log CSV bytes for download, masking SQL for non-managers.
    """
    df = fetch_audit_for_role(role, limit=limit)
